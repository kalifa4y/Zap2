...
router = APIRouter(prefix="/tiktok-live", tags=["TikTok Live"])
...
def fetch_tiktok_live_replay(
    ...
    save_path = os.path.join(settings.STORAGE_RAW_DIR, f"{project_id}_{filename}")

    # Check if a sample raw video exists, else create mock video container
    raw_files = [f for f in os.listdir(settings.STORAGE_RAW_DIR) if f.endswith(".mp4")] if os.path.exists(settings.STORAGE_RAW_DIR) else []
    if raw_files:
        src_file = os.path.join(settings.STORAGE_RAW_DIR, raw_files[0])
